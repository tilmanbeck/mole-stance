...
def create_datasets(args, tokenizer, model_config):
    ...
    for subset_name, task_datasets in datasets.items():
        for dataset in task_datasets:
            ...
            id2label = TASK_MAPPINGS[task_name]["id2label"]
            data_iters[subset_name].append(
                data_iterators.StanceDatasetMTL(
                    dataset, tokenizer, args.max_seq_length, id2label, labelmaps["task2labels"]
                )
            )
            print_subset_statistics(dataset, id2label, subset_name)
        ...
        for step in epoch_iterator:
            ...
            while not batch:
                try:
                    ...
                    task_name = train_datasets[dataset_id].task_name
                    batch = next(dataloaders[dataset_id])
                except StopIteration:
                    pass
            task_id = model.config.task2id[task_name]
            ...
            logits = local_to_global_labels(
                logits.argmax(-1).detach().cpu().numpy(), model.config.task2labels[task_id]
            )
...
def evaluate(
    ...
    with tqdm(
        ...
        for dataset_id, dataloader in enumerate(dataloaders):
            task_name = datasets[dataset_id].task_name
            task_id = model.config.task2id[task_name]
            for batch in dataloader:
                ...
                predictions["pred_stance"] += local_to_global_labels(
                    probs.argmax(-1), model.config.task2labels[task_id]
                ).tolist()

                predictions["true_stance"] += local_to_global_labels(
                    batch["labels"].detach().cpu().tolist(), model.config.task2labels[task_id]
                ).tolist()
